...
def select_data(XX,YY, ctype, min_samples):
    ...
    print(YY.columns, flush = True)
    YY['superdiagnostic'] = YY['superdiagnostic'].apply(parse_superdiag)
    filtered = YY[YY['superdiagnostic'].apply(lambda x: len(x) > 0)]
# Extract the column as a list
    superdiagnostic_list = filtered['superdiagnostic'].tolist()
    print('yy, ', superdiagnostic_list[25:55], flush = True)
    print('lenth,', len(superdiagnostic_list), flush = True)
    if ctype == 'diagnostic':
        ...
        print('diagnose, ' , y[25:55], flush = True)
    elif ctype == 'form':
        # filter
