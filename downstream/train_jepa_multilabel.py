...
def parse():
    ...
    parser.add_argument("--epochs_resnet", type=float, default=10,
                        help="number of epochs")
    parser.add_argument("--blr", type=float, default=1e-4,
                        help="learning rate")
    parser.add_argument("--model_config", type=str, default='jepa',
                        help="model config")                        
    parser.add_argument(
        "--output_dir",
...
def main(config):
    ...
    run_name = f"Model_{config['model_config']}_lr_{hparams['lr']}_dropout_{hparams['dropout']}_batchsize_{hparams['batch_size']}_ckpt_{ckpt_name}_data_{data_name}_data_percentage_{hparams['data_percentage']}"
    log_dir = Path("/home/nadja/MVO_Project_multilabel/TB_JEPA") / run_name
    writer = SummaryWriter(log_dir=str(log_dir))
    #dir_new = os.path.join(log_dir, "resnet")
    #os.makedirs(dir_new, exist_ok=True)
    result_dir = os.makedirs("/home/nadja/MVO_Project_multilabel/predictions_JEPA/", exist_ok = True)
    result_subdir = Path("/home/nadja/MVO_Project_multilabel/predictions_JEPA/") / run_name
    ...
    for files in sorted(
        ...
        model = FinetuningClassifier(encoder, encoder_dim=embed_dim, dropout_rate = config['dropout'], num_labels=n).to(
            device
        )
        lr = config["blr"] 
       ...
       # for epoch in range(1):
        ...
        if best_model is not None:
            ...
            AUC.append(metrics["MultilabelAUROC"])
            F1.append(metrics["MultilabelF1Score"])
            print(str(result_subdir), flush = True)
            ...
            for k, v in to_save.items():
                try:
                    torch.save({k: v}, f'test_{k}.pt')
                    print(f"✅ Saved {k}", flush = True)
                except Exception as e:
                    print(f"❌ Failed to save {k}: {e}", flush = True)
            ...
            #### save model weights from current finetuning iteration
            model.to("cpu")
            torch.save(
                {
                    "encoder": model.state_dict(),
