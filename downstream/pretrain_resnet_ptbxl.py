...
def main(config):
    ...
    log_dir = "/home/nadja/MVO_Project_multilabel/TB_Resnet_ptbxl/"
    hparams = {
            'lr': config['blr'],
    ...
    data_name = os.path.basename(config['data_name'])    

    run_name = f"Model_{config['model_config']}_lr_{hparams['lr']}_dropout_{hparams['dropout']}_batchsize_{hparams['batch_size']}_data_{data_name}_data_percentage_{hparams['data_percentage']}"
    log_dir = Path("/home/nadja/MVO_Project_multilabel/TB_Resnet_ptbxl") / run_name
  ...
  #  waves_train, labels_train = filter_mi_sttc_normal_only(waves_train, labels_train)
   # waves_test, labels_test = filter_mi_sttc_normal_only(waves_test, labels_test)
    ...
    if config['model_config'] == 152:
        model = ResNet152_1D(num_classes=n_labels, input_channels=8, dropout=dropout).to(device)
    if config['model_config'] == 50:
        model = ResNet50_1D(num_classes=n_labels, input_channels=8, dropout=dropout).to(device)
    ...
    for epoch in range(100):
        ...
        writer.add_scalar(f"Loss_train", train_stats["loss"], epoch)
        writer.add_scalar(f"Loss_valid", valid_stats["loss"], epoch)
        ...
        torch.save(
            ...
            f"{config['output_dir']}/resnet_ptbxl_all_{epoch + 1}.pth",
        )
        model.to("cuda")
    writer.close()
